...
pub trait TokenDistributionModule:
    crate::common_util_functions::CommonUtilFunctions + dharitri_sc_modules::pause::PauseModule
{
    ...
    fn claim_tokens(
        ...
        let mut moax_payment_amount = BigUint::zero();
        ...
        if moax_payment_amount > 0 {
            self.send()
                .direct_moax(&claim_destination, &moax_payment_amount);
        }
        ...
        (moax_payment_amount, output_payments).into()
    }
    ...
    fn distribute_tokens_after_auction_end(
        ...
        if !auction.current_winner.is_zero() {
            ...
            self.distribute_tokens_common(
                &MoaxOrDctTokenIdentifier::dct(nft_type.clone()),
                nft_nonce,
                nft_amount_to_send,
    ...
    fn distribute_tokens_common(
        &self,
        nft_type: &MoaxOrDctTokenIdentifier,
        nft_nonce: u64,
        nft_amount_to_send: &BigUint,
        payment_token_id: &MoaxOrDctTokenIdentifier,
