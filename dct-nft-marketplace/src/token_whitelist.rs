...
pub trait TokenWhitelistModule {
    ...
    fn add_tokens_to_whitelist(&self, tokens_to_add: MultiValueEncoded<MoaxOrDctTokenIdentifier>) {
    ...
    fn remove_tokens_from_whitelist(
        &self,
        tokens_to_remove: MultiValueEncoded<MoaxOrDctTokenIdentifier>,
    ) {
        let mut whitelisted_tokens_mapper = self.whitelisted_tokens();
    ...
    fn require_token_whitelisted(&self, token_id: &MoaxOrDctTokenIdentifier) {
    ...
    fn whitelisted_tokens(&self) -> UnorderedSetMapper<MoaxOrDctTokenIdentifier>;
}
