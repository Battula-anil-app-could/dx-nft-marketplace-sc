...
pub struct Auction<M: ManagedTypeApi> {
    pub auctioned_tokens: DctTokenPayment<M>,
    ...
    + dharitri_sc_modules::pause::PauseModule
{
    ...
    fn auction_token(
        ...
        max_bid: BigUint,
        deadline: u64,
        accepted_payment_token: MoaxOrDctTokenIdentifier,
        ...
        let (nft_type, nft_nonce, nft_amount) = self.call_value().single_dct().into_tuple();
        ...
        let auction = Auction {
            auctioned_tokens: DctTokenPayment::new(nft_type, nft_nonce, nft_amount),
            auction_type,
    ...
    fn send_auction_nft(&self, caller: &ManagedAddress, auction: Auction<Self::Api>) {
        self.transfer_or_save_payment(
            caller,
            &MoaxOrDctTokenIdentifier::dct(auction.auctioned_tokens.token_identifier),
