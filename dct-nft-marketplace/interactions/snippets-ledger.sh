...
deploy() {
  mxpy contract deploy --bytecode=${ROOT}/output/dct-nft-marketplace.wasm \
...
upgrade() {
  mxpy contract upgrade $CONTRACT_ADDRESS --bytecode=${ROOT}/output/dct-nft-marketplace.wasm \
  --ledger --ledger-account-index=$LEDGER_INDEX \
  --proxy=${PROXY} --chain=${CHAIN_ID} --gas-limit=${GAS} \
  --arguments ${BID_PERCENT} \
